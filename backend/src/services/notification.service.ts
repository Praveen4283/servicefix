...
import { NotificationMetadata } from '../types/common';
...
class NotificationService {
  ...
  constructor() {
    ...
    if (process.env.NODE_ENV === 'production') {
      if (!process.env.SMTP_HOST || !process.env.SMTP_PORT || !process.env.SMTP_USER) {
        logger.error('SMTP configuration incomplete in production. Required: SMTP_HOST, SMTP_PORT, SMTP_USER');
        throw new Error('SMTP configuration required in production environment');
      }
    }
  ...
  async initializeEmailTransporter() {
    ...
    try {
      ...
      logger.warn(`Failed to fetch email settings from database: ${errorMessage}. Using environment variables.`);
    }
  }
  ...
  private async loadSettingsFromDatabase(): Promise<void> {
    const result = await query(
      'SELECT smtp_server, smtp_port, smtp_username, smtp_password, email_from_name, email_reply_to, enable_email_notifications FROM settings WHERE category = $1',
      ['email']
    );
  ...
  async createInAppNotification(userId: string, content: NotificationContent): Promise<boolean> {
    try {
      ...
      if (content.metadata) {
        notification.metadata = content.metadata as NotificationMetadata;
      }
      ...
      // Log for debugging
      logger.info(`Created in-app notification for user ${userId}:`, content);
      return true;
    } catch (error) {
  ...
  async deleteNotification(notificationId: string, userId: number): Promise<boolean> {
    try {
      ...
      logger.error(`Error soft-deleting notification ${notificationId} for user ${userId}:`, error);
      return false;
    }
  }
  ...
  async deleteAllUserNotifications(userId: string): Promise<boolean> {
    try {
      ...
      if (isNaN(userIdNum)) {
        logger.error('Invalid userId for deleteAllUserNotifications:', userId);
        return false;
      }
      ...
      logger.info(`Soft-deleted ${result.affected ?? 0} notifications for user ${userId}`);
      return true; // Return true even if 0 were deleted
    } catch (error) {
      logger.error(`Error soft-deleting all notifications for user ${userId}:`, error);
      return false;
    }
  }
